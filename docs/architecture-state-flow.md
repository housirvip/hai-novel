...
下面的示例不是数据库原始 JSON，而是进入 AI 前整理后的“可读 prompt 片段”。
...
prompt:
章节：第001章 灰铁城的废民
章节摘要：陆沉在灰铁城黑市接触到一条异常情报线，第一次意识到自己并不是普通废民。
作者意图：开局要有压迫感，并埋下主角身份异常的第一层钩子。
...
- 题材：科幻玄幻
- 文风：冷峻、压迫、悬疑推进
- 故事前提：蓝星文明在高维文明压制下艰难求存

## 大纲上下文
1. [story] 全书总纲；摘要：陆沉从底层废民走向规则革命者；目标：打破旧秩序；冲突：蓝星、人类文明与高等文明压制；结果：建立新秩序雏形
2. [volume] 卷一 灰铁潮生；摘要：从灰铁城切入主角命运；目标：完成主角第一次觉醒前夜的积累

## 人物上下文
1. 陆沉（protagonist）；所属势力：无；职业：废土拾荒者 / 隐性溯规者适配体；简介：表面是灰铁城底层废民；目标：活下去并查明母亲遗物来源；冲突：被黑市、城防与未知势力同时盯上
2. 苏昭宁（supporting）；所属势力：星火学宫；简介：伪装成黑市买家的情报观察者；目标：确认灰铁城异常样本

## 钩子上下文
已绑定到本章的钩子：
1. 母亲留下的黑玉片（item / pending）；本章动作：setup；计划说明：第一次显出异常共鸣
当前仍活跃的钩子：
1. 周既明失踪真相（mystery / active）；摘要：主角导师旧案背后牵涉蓝星高层封锁
```
...
章节摘要：陆沉在灰铁城黑市接触到一条异常情报线，第一次意识到自己并不是普通废民。
作者意图：开局要有压迫感，并埋下主角身份异常的第一层钩子。
额外指令：强化环境压迫、黑市细节和结尾悬念。

请基于以下 plan 生成章节草稿：
## 本章目标
让陆沉在一次本来只为换口粮的黑市交易中，撞上不该属于灰铁城的信息与力量痕迹。
## 冲突推进
黑市盘查、城防追捕、未知观测者同时形成压力。
## 结尾钩子
黑玉片在血迹与电磁噪声中短暂发亮，只有陆沉看见了那串不属于蓝星旧时代的坐标。

本章已绑定钩子：
1. 母亲留下的黑玉片 / 动作：setup / 类型：item / 状态：pending / 计划：第一次显出异常共鸣

当前关键物品：
1. 黑玉片 / 持有人：陆沉 / 持有方式：hidden / 说明：母亲遗物，平时从不离身

contextText（节选）:
## 人物关系上下文
1. 陆沉 -> 周既白 / mentor；摘要：周既白留下的旧线索仍在暗中影响主角选择

## 最近人物状态
1. 陆沉 / chapter=0；状态：长期被灰铁城底层身份束缚；目标：攒够离城资源

## 最近钩子状态
1. 母亲留下的黑玉片 / chapter=0；进度：pending；说明：此前只被当作普通遗物
```
