...
- Provider 可扩展：OpenAI、Anthropic、Mock 走统一抽象。
...
1. V2 先上“章节状态快照 + 对象状态快照”
2. 如果实现成本过高，再退为“最新状态表 + 更新时间”
...
- mock / openai / anthropic 三者共用同一套命令
...
### `item_state_snapshots`

- `id`：主键，自增数字 ID。表示一条物品状态快照。
- `project_id`：所属项目 ID。
- `item_id`：物品 ID。说明状态属于哪个物品。
- `chapter_id`：对应章节 ID。
- `owner_character_id`：当前持有人角色 ID，可为空。
- `chapter_snapshot_id`：来源章节状态快照 ID。
- `status_summary`：物品状态摘要，例如“已认主”“出现裂纹”“暂时失踪”。
- `location`：物品当前所在地点。
- `created_at`：创建时间。
